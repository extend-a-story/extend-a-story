...
*/
  ...
  if ( empty( $error ) )
  {
    $result = mysql_query( "select EpisodeID, SchemeID, ImageID, IsLinkable, IsExtendable, AuthorMailto, AuthorNotify, " .
                                  "Title, Text, AuthorName, AuthorEmail, EditDate from EpisodeEditLog " .
...
<?php
  ...
  if ( empty( $error ) )
  {
    $result = mysql_query( "select TargetEpisodeID, IsBackLink, Description from LinkEditLog " .
                           "where EpisodeEditLogID = " . $episodeEditLogID . " order by LinkEditLogID" );
    if ( ! $result )
    {
...
?>
<HTML><HEAD>
<TITLE><?php echo( $storyName ); ?>: <?php echo( $title ); ?> [Episode <?php echo( $episode ); ?>] - Edit Log</TITLE>
</HEAD><?php echo( $body ); ?>
